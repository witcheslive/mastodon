...
class ComposeForm extends ImmutablePureComponent {
  ...
  handleSubmit = () => {
    ...
    if (is_submitting || is_uploading || is_changing_upload || length(fulltext) > 666 || (fulltext.length !== 0 && fulltext.trim().length === 0 && !anyMedia)) {
      return;
    }
  ...
  render () {
    ...
    const disabledButton = disabled || this.props.is_uploading || this.props.is_changing_upload || length(text) > 666 || (text.length !== 0 && text.trim().length === 0 && !anyMedia);
    let publishText = '';
